...
# setup vs code ssh server (no password setup). to connect "ssh -p 8023 root@<ip-address>"
apk add gcompat libstdc++ openssh openssh-server
ssh-keygen -A
passwd -d root

echo "/usr/sbin/sshd -p 8023" >> $bashrc # setup autorun
...
# install programming languages support  (this takes an hour to install)
